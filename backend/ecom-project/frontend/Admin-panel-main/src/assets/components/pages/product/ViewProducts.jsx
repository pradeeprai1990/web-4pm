...
const rows = [
  ['Berlin Rattan Bed', 'Beds', '$6,200', '1', 'Active'],
  ['Ambre Vintage Bed', 'Beds', '$5,400', '2', 'Inactive'],
  ['Bohemian Carved Bed', 'Beds', '$4,900', '3', 'Active']
]

export default function ViewProducts() {
  return <StaticView title='View Product' section='Product' columns={['Product Name', 'Category', 'Price', 'Order']} rows={rows} />
}
...
function StaticView({ title, section, columns, rows }) {
  return (
    <section className='w-full'>
      ...
      <div className='p-4'>
        <div className='py-4 relative px-6 my-3 rounded-lg border border-slate-200 w-full bg-white shadow-sm'><p className='font-semibold py-2 text-[20px]'>Filter</p><div className='flex items-end gap-6'><div className='mb-5'><label className='block mb-2 font-medium text-gray-700'>Product Name</label><input className='text-[17px] border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-400 focus:border-indigo-500 block w-full py-2.5 px-3' placeholder='Enter Product Name' /></div><button className='text-white bg-slate-500 px-6 py-2.5 rounded-lg mb-5'>Clear</button><button className='text-white bg-indigo-600 px-6 py-2.5 rounded-lg mb-5'>Apply</button></div></div>
